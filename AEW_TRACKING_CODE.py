...
For questions on the methodology or code, please contact Quinton via email
at quinton.lawton@rsmas.miami.edu.
...
# -----------------------------------------------------------------------------
##### INPUT SETTINGS  #####
#The following variables control the years being ingested, the name of the system
...
#### YEARS OF DATA TO RUN ON #####
year_int = np.arange(1979,2020,1)
year_list = year_int.astype(str)
year_list =  ['2020']
system = 'B1-6hr' #Obsolete now
rad_used = 600
out_attr = '_0924_UPDATE_curvfix' #NAME OF RUN
extra_info = out_attr
...
#         band_len = 6
    ...
    if separate_bands == True:
        lat_extra_st, n = find_nearest(lat, 15)
        lat_extra_end, n = find_nearest(lat,5)
        ...
        for i in range(band_len): #We will have six bands
            lat_avg_st, n = find_nearest(lat, 15+i) #We only want to average the cells in a 5-20N range
            lat_avg_end, n = find_nearest(lat, 5+i)
...
# ITERATE OVER ALL THE YEARS IN THE GIVEN LIST
for year in year_list:
    # DATA FILE TO LOAD IN
    data_dir = '../DATA/CURV_VORT/RADIAL_AVG/'
    
    datafile = 'curv_vort_helmholz_700_R'+str(rad_used)+'_'+year+'_'+system+'.nc'
    wind_dir = '../ERA5/HELMHOLTZ/wind_season_helmholtz_wind_'+year+'_'+system+'.nc'
    
    #datafile = 'curv_vort_700_R'+str(rad_used)+'_'+year+'_'+system+'.nc'
    #wind_dir = '../ERA5/COMPILED/wind_season_lowres_700_'+year+'_'+system+'.nc'
    # First, import the data
    nc_file = Dataset(data_dir+datafile, 'r')
    #nc_file
   ...
   # --------------------------------------------------------------------------
    ...
    AEW_lon_filter = savgol_filter(AEW_lon, smooth_len, 2)
    AEW_lat_filter = savgol_filter(AEW_lat, smooth_len, 2)
    ...
    reg_list = [num2date(i, time_units, only_use_cftime_datetimes = False) for i in time]
    file_info = str(year)+'_'+str(system)
    info_name = file_info+'_'+str(thres_init*10**6)+'init_'+str(thres_cont*10**6)+'cont_'+str(centroid_rad)+'rad_'+str(centroid_it)+'it_centroid_'+extra_info
    print(info_name)
    # -------------------------------------------------------------------------
    # END DATA CLEANUP
    ...
    # -------------------------------------------------------------------------
    if save_data == True:
        outfile = '../DATA/AEW/TRACKS/AEW_data_'+out_attr+str(year)+'_'+str(system)+'.nc'
